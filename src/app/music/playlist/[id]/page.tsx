...
import { ArrowLeft, Play, Trash2, GripVertical, Camera, Heart } from "lucide-react";
    ...
    >
      <div className="flex items-center gap-4 flex-1">
        ...
        >
          ...
          <Play size={16} className="text-white w-6 hidden group-hover:block" fill="currentColor"/>
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          title={favourites.find((f) => f.id === song.id) ? "Remove from favourites" : "Add to favourites"}
        >
          <Heart 
            size={18} 
            fill={favourites.find((f) => f.id === song.id) ? "currentColor" : "none"}
          />
        </button>
        <button
          onClick={(e) => {
...
const PlaylistPage = ({ params }: { params: Promise<{ id: string }> }) => {
  const { id } = use(params);
  const { playlists, playSong, removeFromPlaylist, reorderPlaylist, updatePlaylistImage, toggleFavourite, favourites } = useMusic();
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id && playlist) {
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        reorderPlaylist(playlist.id, arrayMove(playlist.songs, oldIndex, newIndex));
      }
    }
  };
  ...
  if (!playlist) {
    return (
      <div className="text-white p-6">
        ...
        <Link href="/music" className="text-red-500 hover:underline flex items-center gap-2">
  ...
  return (
    <div className="w-[96%] mx-auto px-2 pb-32 pt-6">
      <div className="mb-8">
        <Link href="/music" className="text-gray-400 hover:text-white flex items-center gap-2 mb-4 transition-colors">
        ...
        <div className="flex items-end gap-6">
           ...
           >
              {playlist.image ? (
                ...
                <span className="text-4xl font-bold text-white">{playlist.name.charAt(0)}</span>
              )}
      ...
      <div className="bg-gray-900/50 rounded-lg p-4">
        {playlist.songs.length === 0 ? (
          <div className="text-center py-10 text-gray-500">
            ...
            <Link href="/music" className="text-red-500 hover:underline mt-2 inline-block">
              Find songs to add
            </Link>
          </div>
        ) : (
